...
document.addEventListener('DOMContentLoaded', () => {
    // --- КОНСТАНТЫ ---
    const ITEMS = { 'beer':'ПИВО', 'cig':'СИГАРЕТЫ', 'cuff':'НАРУЧНИКИ', 'saw':'ПИЛА', 'mag':'ЛУПА', 'phone':'ТЕЛЕФОН', 'inv':'ИНВЕРТОР', 'adren':'АДРЕНАЛИН', 'pills':'ТАБЛЕТКИ' };
    ...
    const ui = {
        screens: { name: el('screen-name'), mode: el('screen-mode'), cash: el('screen-cashout'), end: el('screen-end'), game: el('game') },
        playerNameInput: el('inp-name'), playerNameDisplay: el('p-name'),
        hp: { p: el('p-hp'), d: el('d-hp') }, hpNum: { p: el('p-hp-num'), d: el('d-hp-num') },
        ...
        aiStatus: el('ai-status'), gameTitle: el('game-title'),
        ...
        dealerNameContainer: el('d-box').querySelector('.character-header div') // Ссылка на контейнер имени
    };
    ...
    let currentScale = 1;
    function setupAudioAnalysis(audioElement) { try { if (!audioContext) audioContext = new (window.AudioContext || window.webkitAudioContext)(); if (!analyser) { analyser = audioContext.createAnalyser(); analyser.fftSize = 64; analyser.smoothingTimeConstant = 0.5; } if (!audioElement._source) { audioElement._source = audioContext.createMediaElementSource(audioElement); audioElement._source.connect(analyser); analyser.connect(audioContext.destination); } } catch (e) {} }
    function resumeAudioContext() { if (audioContext && audioContext.state === 'suspended') audioContext.resume(); }
    function analyzeMusic() { if (!analyser || !currentTrack || currentTrack.paused) { ui.vignette.style.boxShadow = 'inset 0 0 100px 20px rgba(0,0,0,0.3)'; ui.gameContainer.style.transform = 'scale(1)'; requestAnimationFrame(analyzeMusic); return; } const dataArray = new Uint8Array(analyser.frequencyBinCount); analyser.getByteFrequencyData(dataArray); const bass = (dataArray[0] + dataArray[1] + dataArray[2]) / 3; const threshold = 160; if (bass > threshold) { const strength = (bass - threshold) / (255 - threshold); const targetScale = 1 + (strength * 0.02); currentScale += (targetScale - currentScale) * 0.2; const alpha = strength * 0.5; let color = state.wiresCut ? `255, 0, 0` : `79, 139, 79`; ui.vignette.style.boxShadow = `inset 0 0 150px 50px rgba(${color}, ${alpha})`; } else { currentScale += (1 - currentScale) * 0.1; ui.vignette.style.boxShadow = 'inset 0 0 100px 20px rgba(0,0,0,0.3)'; } ui.gameContainer.style.transform = `scale(${currentScale})`; requestAnimationFrame(analyzeMusic); }
    function playMusic(track) { if (!isAudioInitialized) return; resumeAudioContext(); if (currentTrack === track) return; for (const key in ui.audio) { fadeOut(ui.audio[key]); } currentTrack = track; if (track) { setupAudioAnalysis(track); track.currentTime = 0; fadeIn(track); } }
    function playSfx(name) { if (ui.sfx[name]) { ui.sfx[name].currentTime = 0; ui.sfx[name].play().catch(()=>{}); } }
    function fadeIn(audioElement) { audioElement.volume = 0; var playPromise = audioElement.play(); if (playPromise !== undefined) { playPromise.then(_ => {}).catch(error => {}); } let vol = 0; const i = setInterval(() => { vol = Math.min(1, vol + 0.1); audioElement.volume = vol; if(vol===1) clearInterval(i); }, 50); }
    function fadeOut(audioElement) { if(audioElement.paused) return; let vol = audioElement.volume; const i = setInterval(() => { vol = Math.max(0, vol - 0.1); audioElement.volume = vol; if(vol===0) { clearInterval(i); audioElement.pause(); } }, 50); }
    function initAudio() { if (!isAudioInitialized) { audioContext = new (window.AudioContext || window.webkitAudioContext)(); isAudioInitialized = true; resumeAudioContext(); playMusic(ui.audio.lobby); analyzeMusic(); } else { resumeAudioContext(); } }
    ...
    function resetWatchdog() { if (dealerWatchdogTimer) clearTimeout(dealerWatchdogTimer); if (state.turn === 'd' && !state.cuffedTurns.d && !state.isGameOver) { dealerWatchdogTimer = setTimeout(() => { log("Дилер замешкался... (АВТО-ХОД)", "l-sys"); ui.boxes.d.classList.remove("thinking"); ui.dealerNameContainer.innerText = "ДИЛЕР"; const target = aiMemory.knownCharge === 'blank' ? 'self' : 'p'; shoot(target); }, 8000); } }
    ...
    function initializeStage() {
        ...
        if (state.mode === GAME_MODES.STANDARD) {
            ...
            switch(state.stage) {
                case 1: currentMaxHp = 2; giveItems = false; log("--- ЭТАП 1: Предметы отключены ---", "l-sys"); playMusic(getNormalRoundMusic()); break;
                case 2: currentMaxHp = 4; log("--- ЭТАП 2: Предметы доступны ---", "l-sys"); playMusic(getNormalRoundMusic()); break;
                case 3: currentMaxHp = 6; state.isSuddenDeath = true; ui.gameTitle.innerText = "ФИНАЛЬНЫЙ ЭТАП"; log("--- ФИНАЛ: Системы жизнеобеспечения активны ---", "l-sys"); playMusic(getFinalRoundMusic()); break;
            }
        } else {
            ui.gameTitle.innerText = `БЕСКОНЕЧНЫЙ - РАУНД ${state.stage}`;
            if ((state.stage - 1) % 3 === 0) { state.endlessCycleStartMoney = state.money; state.endlessCycleStartStage = state.stage; log(`--- НОВЫЙ ЦИКЛ (Раунды ${state.stage}-${state.stage+2}) ---`, "l-sys"); }
            if (state.stage % 3 === 0) {
                state.isSuddenDeath = true; log("--- СМЕРТЕЛЬНЫЙ РАУНД: Системы активны ---", "l-sys");
                playMusic(getFinalRoundMusic()); currentMaxHp = 6;
            } else {
                playMusic(getNormalRoundMusic()); currentMaxHp = 4;
            }
        }
    ...
    async function aiTurn() {
        ...
        ui.dealerNameContainer.innerText = "ДУМАЕТ...";
        ...
        if(state.cuffedTurns.d > 0) { 
            ...
            state.cuffedTurns.d = 0; state.turn = 'p'; 
            // Отключаем таймер, так как ход переходит к игроку
        ...
        while(itemUsedInLoop) {
            ...
            else if (!state.wiresCut && state.hp.d <= state.maxHpForRound / 2 && (state.items.d.cig > 0 || state.items.d.pills > 0)) { actionToTake = state.items.d.cig > 0 ? "cig" : "pills"; }
        ...
        ui.dealerNameContainer.innerText = "ДИЛЕР";
